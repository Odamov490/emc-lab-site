// src/pages/Login.jsx (enhanced + Standartlar + Chat + AI)
...
const ORG_LIST = [
  "Toshkent",
  "Attest",
  "Premier Certification Center",
  "Electro-Class Control",
];
...
const T = {
  uz: {
    ...
    total: "Jami",
    inprog: "Jarayonda",
    done: "Sinov tugatildi",
    canceled: "Bekor qilindi",
    payyes: "To‘lov bor",
    payno: "To‘lov yo‘q",
    newApp: "Yangi ariza",
    appNum: "Ariza raqami",
    org: "Organ Sertifikatsiya",
    product: "Mahsulot",
    client: "Pskent/Toshkent (mijoz)",
    payStatus: "Status (to‘lov)",
    flowStatus: "Status (holat)",
    redZone: "Qizil zona",
    note: "Izoh",
    add: "Qo‘shish",
    save: "Saqlash",
    ...
    updated: "Yangilandi",
    deleted: "O‘chirildi",
    changePass: "Parolni almashtirish",
    newPass: "Yangi parol",
    confirm: "Tasdiqlash",
    passChanged: "Parol almashtirildi",
    duplicate: "Bu ariza raqami allaqachon mavjud",
    sort: "Saralash",
    stdTitle: "Standartlar to‘plami",
    stdHint:
      "/public/standards/ ichiga fayllarni joylang va index.json ni to‘ldiring",
    stdRefresh: "Yangilash",
    stdDownload: "Yuklab olish",
    stdOpenFolder: "Papkani ochish",
    stdCount: "Jami fayl",
    stdBadJson: "index.json xato yoki to‘liq emas",
    stdEmpty: "Hozircha standartlar topilmadi",
  },
  ru: {
    title: "Вход",
    username: "Логин",
    ...
    payStatus: "Статус (оплата)",
    flowStatus: "Статус (этап)",
    ...
    duplicate: "Такая заявка уже существует",
    sort: "Сортировка",
    stdTitle: "Каталог стандартов",
    stdHint:
      "Положите файлы в /public/standards/ и заполните index.json",
    stdRefresh: "Обновить",
    stdDownload: "Скачать",
    stdOpenFolder: "Открыть папку",
    stdCount: "Всего файлов",
    stdBadJson: "index.json поврежден или неполный",
    stdEmpty: "Пока нет стандартов",
  },
};
...
function Card({ children, className = "" }) {
  return (
    <div
      className={`rounded-2xl border border-black/10 bg-white/80 dark:bg-white/10 backdrop-blur p-5 shadow ${className}`}
    >
      {children}
    </div>
  );
...
function Pill({ children }) {
  return (
    <span className="inline-flex items-center rounded-full px-3 py-0.5 text-xs bg-sky-100 text-sky-800">
      {children}
    </span>
  );
}
const Input = (p) => (
  <input
    {...p}
    className={`mt-1 w-full rounded-xl border px-3 py-2 ${p.className || ""}`}
  />
);
...
function validateApp(app) {
  ...
  if (!/^\d{3,}$/.test(app.appNum.trim()))
    errors.push(
      "Ariza raqami faqat raqam va kamida 3 belgidan iborat bo‘lsin."
    );
...
export default function Login() {
  ...
  const [lang, setLang] = useState(
    () => localStorage.getItem("emc_lang") || "uz"
  );
  ...
  const [fPay, setFPay] = useState(t.all);
  const [fFlow, setFFlow] = useState(t.all);
  const [fRed, setFRed] = useState(t.all);
 ...
 const doLogin = async (e) => {
  ...
  try {
    ...
    setMe(auth);
    setSubmitting(false);
    setTab("profile");  // ← ENDIIII PROFILGA O‘TADI
  } catch (e2) {
    console.error(e2);
  ...
  const addApp = async (e) => {
    e.preventDefault();
    if (!me) return;
    const errs = validateApp(appForm);
    if (errs.length) {
      alert(errs.join("\n"));
      return;
    }
    ...
    try {
      ...
      const dupQ = query(
        ...
        where("appNum", "==", appForm.appNum.trim()),
        limit(1)
      );
  ...
  const startEdit = (row) => {
    ...
    setEditForm({
      appNum: row.appNum || "",
      org: row.org || ORG_LIST[0],
      product: row.product || "",
      client: row.client || "",
      pay: row.pay || STATUS_TOLOV[0],
      flow: row.flow || STATUS_HOLAT[0],
      red: row.red || QIZIL_ZONA[1],
      note: row.note || "",
    });
  };
  const doUpdate = async () => {
    if (!editing) return;
    const errs = validateApp(editForm);
    if (errs.length) {
      alert(errs.join("\n"));
      return;
    }
    setUpdating(true);
    try {
      await updateDoc(doc(db, "applications", editing.id), {
        ...editForm,
  ...
  // inline quick setters for status
  const canQuickEdit = (row) =>
    me?.role === "admin" || row.byUserId === me?.id;
  const setFlow = async (row, value) => {
    try {
  ...
  const addEmployee = async (e) => {
    ...
    const body = { ...empForm };
    if (!body.photoUrl && STAFF_PHOTOS[body.fullname])
      body.photoUrl = STAFF_PHOTOS[body.fullname];
    if (
      !body.username.trim() ||
      !body.password.trim() ||
      !body.fullname.trim()
  ...
  const changeMyPass = async () => {
    if (!me) return;
    if (!newPass.trim()) return alert("Parol kiriting");
    setChanging(true);
    try {
  ...
  const loadStandards = async () => {
    ...
    try {
      ...
      try {
        ...
        throw new Error(
          t.stdBadJson + ` (JSON.parse)\n` + parseErr.message
        );
      }
      if (!Array.isArray(json))
        throw new Error(t.stdBadJson + ` (Array expected)`);
      const ok = json.every(
        (x) => x && typeof x.id !== "undefined" && x.name && x.file
      );
      if (!ok)
        throw new Error(t.stdBadJson + ` (fields: id,name,file)`);
      setStdList(json);
    } catch (e) {
      console.error(e);
            ...
            <Card>
              ...
              <div className="mt-5 border-t pt-4">
                ...
                <div className="flex gap-2 max-w-sm">
                  ...
                  />
                  <button
                    onClick={changeMyPass}
                    disabled={changing}
                    className="rounded-xl bg-sky-600 text-white px-4 py-2 text-sm hover:opacity-90 disabled:opacity-60"
                  >
                    {t.confirm}
                  </button>
                </div>
            ...
            <>
              ...
              <Card>
                ...
                <div className="flex flex-wrap gap-2 text-sm">
                  ...
                  <Pill>
                    {t.inprog}: {stat.inprog}
                  </Pill>
                  <Pill>
                    {t.done}: {stat.done}
                  </Pill>
                  <Pill>
                    {t.canceled}: {stat.canceled}
                  </Pill>
                  <Pill>
                    {t.payyes}: {stat.payyes}
                  </Pill>
                  <Pill>
                    {t.payno}: {stat.payno}
                  </Pill>
              ...
              <Card>
                ...
                >
                  <div>
                    ...
                    <Input
                      value={appForm.appNum}
                      onChange={(e) =>
                        setAppForm((s) => ({
                          ...s,
                          appNum: e.target.value,
                        }))
                      }
                      placeholder="4654563"
                      required
                  ...
                  <div className="sm:col-span-2">
                    ...
                    <div className="mt-3 flex flex-wrap gap-2">
                      ...
                      <button
                        ...
                        onClick={() => {
                          const header =
                            "appNum,org,product,client,pay,flow,red,note\n";
                          const body = filtered
                            .map((a) =>
                              [
                                a.appNum,
                    ...
                    <thead>
                      <tr className="text-left text-gray-500">
                        <th className="py-2 pr-3">
                          <SortBtn col="appNum" label={t.appNum} />
                        </th>
                        <th className="py-2 pr-3">
                          <SortBtn col="org" label={t.org} />
                        </th>
                        <th className="py-2 pr-3">
                          <SortBtn col="product" label={t.product} />
                        </th>
                        <th className="py-2 pr-3">
                          <SortBtn col="client" label={t.client} />
                        </th>
                        <th className="py-2 pr-3">
                          <SortBtn col="pay" label={t.payStatus} />
                        </th>
                        <th className="py-2 pr-3">
                          <SortBtn col="flow" label={t.flowStatus} />
                        </th>
                        <th className="py-2 pr-3">
                          <SortBtn col="red" label={t.redZone} />
                        </th>
                        <th className="py-2 pr-3">{t.note}</th>
                        <th className="py-2 pr-3">{t.user}</th>
                        <th className="py-2 pr-3">
                          <SortBtn col="createdAt" label={t.time} />
                        </th>
                ...
                >
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      placeholder="https://...jpg (ixtiyoriy)"
                    />
                  </div>
                  <div>
                    <label className="font-medium">
              ...
              <div className="space-y-3 text-sm">
                ...
                {movements.slice(0, 50).map((m) => (
                  <div key={m.id} className="flex items-start gap-3">
                    <div className="mt-1">
                      <Pill>{m.type === "in" ? "Kirim" : "Chiqim"}</Pill>
                    </div>
                    <div>
            ...
            <Card>
              ...
              <div className="flex items-center gap-2 mb-3">
                <input
                  ...
                  placeholder={t.search}
                  value={stdQ}
                  onChange={(e) => setStdQ(e.target.value)}
                />
                <Pill>
                  {t.stdCount}: {stdFiltered.length}
                </Pill>
              </div>
                  ...
                  >
                    ...
                    <div className="flex gap-2">
                      <a
                        href={s.file}
                        download
                        className="rounded-lg bg-sky-600 text-white px-3 py-1.5 text-xs hover:opacity-90"
                      >
                        {t.stdDownload}
                      </a>
                      <a
